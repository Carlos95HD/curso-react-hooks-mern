...
describe("Pruebas en useAuthStore", () => {
  ...
  test("startRegister debe crear un usuario", async () => {
    ...
    //Crear Espía para la creación de usuarios en la base de datos
    ...
    expect({ errorMessage, status, user }).toEqual({
      errorMessage: undefined,
      status: "authenticated",
      user: { name: "Test User", uid: "12312312" },
    });
  ...
  test("starRegister debe fallar en la creación", async () => {
    const mockStore = getMockStore({ ...notAuthenticatedState });
    const { result } = renderHook(() => useAuthStore(), {
      wrapper: ({ children }) => (
        <Provider store={mockStore}>{children}</Provider>
    ...
    expect({ errorMessage, status, user }).toEqual({
      errorMessage: "Email ya esta registrado por otro usuario",
      status: "not-authenticated",
      user: {},
    });
  });
  ...
  test("checkAuthToken debe de autenticar el usuario si hay un token", async () => {
    ...
    expect({ errorMessage, status, user }).toEqual({
      errorMessage: undefined,
      status: "authenticated",
      user: { name: "Test User", uid: "635c757e0df8a61c096bcc3f" },
    });
  });
});
